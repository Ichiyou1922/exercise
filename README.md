...
## Lorenz_Attractor
- Lorenz_Attractorを再現するプログラム．
- Lorenz Attractorのルール
1. $\frac{dx}{dt} = \sigma (y - x)$
2. $\frac{dy}{dt} = x(\rho -z)-y$
3. $\frac{dz}{dt} = xy- \beta z$
4. xは対流の速度，yは水平方向の温度差，zは垂直方向の温度差を表す．

- ルンゲ・クッタ法
  - 今回4次
  1. $k_{1}=f(x_{n}, t_{n})$
  2. $k_{2}=f(x_{n} + h \frac{k_{1}}{2}, t_{n} + \frac{h}{2})$
  3. $k_{3}=f(x_{n} + h \frac{k_{2}}{2}, t_{n} + \frac{h}{2})$
  4. $k_{4} = f(x_{n}+hk_{3}, t_{n}+h)$
  5. $x_{n+1} = x_{n} + \frac{h}{6} (k_{1}+2k_{2}+2k_{3}+k{4})$
  - 1ステップで4回シミュレーションを行っている． $x_{n}$の右辺2項目は*重み付き平均*．

## Category_Composition
- 圏論における結合律，単位律を確認するプログラム.
  - 結合律を確認する関数composeは関数を引数に設定したがlambda式で使うのはx...
  - composeが受け取る引数f, gの引数はxだからその情報を引き継いでいい......のだろうか
